...
class ImageAnalyzerApp:
    ...
    def init_tab2(self):
        ...
        self.params = {
            ...
            'step_10_final': {'min_area': 100, 'bbox_color': 'green'}
        }
    ...
    def process_all_steps(self):
        ...
        # Step 10: Final Result with Bounding Boxes
        result = self.step_data['masked_bgr'].copy()

        bbox_color_name = self.params['step_10_final']['bbox_color']
        color_map = {
            'red': (0, 0, 255),
            'green': (0, 255, 0),
            'blue': (255, 0, 0),
        ...
        bbox_color = color_map.get(bbox_color_name, (0, 255, 0))

        min_area = self.params['step_10_final']['min_area']

        # Draw bounding boxes for each labeled region
    ...
    def display_current_step(self):
        ...
        step_descriptions = [
            ...
            "Final result with bounding boxes around detected regions."
        ]
    ...
    def create_step10_controls(self):
        ...
        area_frame = ttk.Frame(self.params_frame)
        area_frame.pack(fill='x', pady=2)

        area_var = tk.IntVar(value=self.params['step_10_final']['min_area'])
        area_scale = ttk.Scale(area_frame, from_=50, to=500, orient='horizontal',
                              variable=area_var,
                              command=lambda v: self.update_step10_area(area_var))
        area_scale.pack(side='left', fill='x', expand=True, padx=(0, 5))

        area_spinbox = ttk.Spinbox(area_frame, from_=50, to=500, textvariable=area_var,
                                   width=6, command=lambda: self.update_step10_area(area_var))
        area_spinbox.pack(side='right')
        area_spinbox.bind('<Return>', lambda e: self.update_step10_area(area_var))
        area_spinbox.bind('<FocusOut>', lambda e: self.update_step10_area(area_var))

        ttk.Label(self.params_frame, text="Bounding Box Color:").pack(anchor='w', pady=(10, 2))
        bbox_var = tk.StringVar(value=self.params['step_10_final']['bbox_color'])
        colors = ['red', 'green', 'blue', 'yellow', 'cyan', 'magenta']
        bbox_combo = ttk.Combobox(self.params_frame, textvariable=bbox_var,
                                  values=colors, state='readonly')
        bbox_combo.pack(fill='x', pady=2)
        bbox_combo.bind('<<ComboboxSelected>>', lambda e: self.update_step10_color(bbox_var))

        self.param_widgets = {'area_var': area_var}
    ...
    def update_step10_area(self, area_var):
    ...
    def update_step10_color(self, bbox_var):
        """Update step 10 bbox color"""
        self.params['step_10_final']['bbox_color'] = bbox_var.get()
        self.process_all_steps()
        self.display_current_step()
    ...
    def tab3_process_single_image(self, image_path, defects_folder, non_defects_folder, filename):
        ...
        bbox_color_name = params['step_10_final']['bbox_color']
        color_map = {
            'red': (0, 0, 255),
            'green': (0, 255, 0),
            'blue': (255, 0, 0),
